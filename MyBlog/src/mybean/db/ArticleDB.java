...
public class ArticleDB {
	...
	public void updateRecord(Article art) throws SQLException {
		String sql = "UPDATE user SET id=?, name=?, pwd=?, gender=? WHERE idx=?";
		pstmt = con.prepareStatement(sql);
		pstmt.setString(1, art.getId());
		pstmt.setString(2, art.getName());
		pstmt.setString(3, art.getPwd());
	...
	public Article getRecord(int idx) throws SQLException {
		String sql = "SELECT id,name,pwd,gender FROM user WHERE idx=?";
		pstmt = con.prepareStatement(sql);
		pstmt.setInt(1, idx);
		...
		art.setGender(rs.getString("gender"));
		art.setIdx(idx);
		return art;
	}
	...
	public Article searchRecord(String id) throws SQLException{
